...
class PredisStoreTest extends TestCase
{
    private PredisStore $store;
    ...
    protected function setUp(): void
    {
        ...
        try {
            $this->store = new PredisStore(
                $this->createPredisConnection(),
                prefix: 'kv_predis_test_'.uniqid().'_',
            );
        } catch (\Throwable) {
    ...
    public function test_constructor_accepts_predis_instance(): void
    {
        $store = new PredisStore($this->createPredisConnection(), prefix: 'kv_ctor_'.uniqid().'_');
    ...
    {
        $otherStore = new PredisStore($this->createPredisConnection(), prefix: 'other_'.uniqid().'_');
        $otherStore->set('key', 'other');

        $this->store->set('key', 'mine');

        $this->assertSame('mine', $this->store->get('key'));
        $this->assertSame('other', $otherStore->get('key'));

        $otherStore->clear();
    }

    public function test_clear_only_removes_prefixed_keys(): void
    {
        $otherStore = new PredisStore($this->createPredisConnection(), prefix: 'other_'.uniqid().'_');
        $otherStore->set('survive', 'yes');

        $this->store->set('a', 1);
        $this->store->clear();

        $this->assertFalse($this->store->has('a'));
        $this->assertTrue($otherStore->has('survive'));

        $otherStore->clear();
    }

    private function createPredisConnection(): \Predis\ClientInterface
    {
